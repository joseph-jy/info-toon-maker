...
긴 서사를 **한 장의 세로 포스터**, **한 페이지짜리 세로 웹툰 인포그래픽**, 또는 **2-8페이지 성인 학습만화 시리즈**로 설계하는 하네스다. 짧은 주제를 한 장의 캐릭터 설명 만화로 만드는 전용 흐름은 `one-page-comic-toon`을 사용한다.
...
5. Run the Korean copy voice pass over every baked string before freezing the whitelist, following `references/korean-copy-voice-rules.md`. Drop the AI signature phrases, count parallelism and colon subtitles series-wide, keep each character's register and ending set fixed, check that the explainer's mechanism bubbles are literal statements in domain terms (explainer analogies at most one per page, each after the literal line; learner and challenger lines are exempt), and preserve the hedges the claim ledger requires. Record it under `## Korean Copy Voice Pass` in `imagegen-checklist.md`.
...
2. Fill `imagegen-checklist.md` with:
   ...
   - the Korean copy voice pass result (S1 patterns remaining, register consistency, explainer direct-statement check and analogy counts, series-wide counts, hedges kept for claim status)
