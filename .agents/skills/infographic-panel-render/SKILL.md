...
- whitelist를 확정하기 **직전에** 어투 패스를 한 번 돌린다. 전체 규칙과 패턴 ID는 `references/korean-copy-voice-rules.md`.
  ...
  - 설명자 말풍선은 직접 서술인지 본다. 메커니즘·주장 풍선은 글자 그대로 읽어 그 시스템에 대해 참이어야 하고(주어가 실제 대상, 동작이 있는 그대로, 조건 명시), 설명자 비유는 페이지당 최대 1개·시리즈 최대 3개이며 직접 서술 문장 뒤에만 온다. 자료 인서트·다이어그램 라벨·제목은 분야 용어를 쓴다. 학습자·반론자의 비유는 손대지 않는다. 결과는 `## Copy Voice Contract`의 설명자 항목에 적는다.
