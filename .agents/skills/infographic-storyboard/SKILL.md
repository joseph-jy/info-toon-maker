...
- 설명자는 직접 서술한다. 메커니즘과 주장은 실제 대상(LLM, 모델, 프롬프트)을 주어로, 실제 동작과 조건을 그 분야의 용어로 말한다. 비유·의인화·치환어("이름표", "다른 집", "버릇")는 학습자·반론자·마스코트의 자리이고, 설명자 말풍선에서는 페이지당 최대 1개, 직접 서술 문장 뒤에만 둔다. 규칙과 예시: `references/korean-copy-voice-rules.md`의 "설명자 직접 서술".
...
- 설명자 말풍선은 치환 테스트를 통과해야 한다. 글자 그대로 읽어 그 시스템에 대해 참이면 직접 서술이고, 독자가 다른 대상으로 바꿔 읽어야 참이면 비유다. 자료 인서트·다이어그램 라벨·페이지 제목은 항상 분야 용어를 쓴다. 학습자·반론자의 비유와 농담은 그대로 둔다 — 가벼움은 거기서 나온다.
...
- 설명자가 메커니즘을 비유로만 말하게 두지 말 것. "이름표가 보이면 후해지니까 떼고, 심판은 다른 집에서 데려와요"가 아니라 "LLM은 같은 모델이 생성한 텍스트를 더 후하게 평가하는 경향이 있어요"다. 비유는 학습자와 반론자의 자리다
...
- 설명자 말풍선이 치환 테스트를 통과하는가(글자 그대로 그 시스템에 대해 참인가), 설명자 비유가 페이지당 1개 이하이고 직접 서술 뒤에 오는가, 자료 인서트·라벨·제목이 분야 용어인가
