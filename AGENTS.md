...
- **Confusable cast -> change the run, not the profile.** When two cast members would be hard to tell apart in the same series (same age band, same hair length and color, same outfit silhouette), give one of them a distinguishing appearance change **scoped to that run** and leave `cast/<id>.character.yaml` untouched. This is the one sanctioned exception to the rule above, and it must be recorded rather than applied silently.
  - The trigger is **co-appearance**, not the profile being wrong. The profile still describes the real person accurately, and a run that casts only one of them has nothing to disambiguate — editing the profile would degrade likeness in every future run to fix a problem that exists in one series. Change the profile only when the real person's appearance actually changed; that is a stale-profile fix, not an override, so ask which case it is instead of assuming.
  - The unit of override is the **run, not the panel**. Hold the changed appearance constant across `character_sheet` and every page. The sheet locks the look and pages inherit it, so toggling per panel cannot work and only produces identity drift — and it defeats the purpose, since readers identify a character by the look they saw on the previous page.
  - Record it in `character-bible.md` next to that character's tokens: instruction source, trigger condition, run-not-panel scope, what the override buys, what it costs in likeness, and the fact that the profile was not edited. Rotation and likeness audits both read this file, so an unrecorded override is invisible.
  - When the override contradicts an identity reference photo, restate it in **three** places or the render will inherit the photo: the shared prompt policy's cast block, the `PHOTO IDENTITY REFERENCE RULE` (state explicitly that the character bible wins on this attribute), and the `character_sheet` slot prompt. Then verify it at full resolution during sheet review — the sheet's version propagates to every page.
  - Check the new appearance against the palette before committing to it. A color introduced for character identity must not collide with a color the run reserved for meaning (warning red, accent amber). If it does, pick a separated value, write the separation rule into `layout-bible.md`, and keep the semantic color away from that character in panel staging.
  - Prefer changing a **non-identity-critical** attribute. Hair color, an added accessory, or a top color are cheap; face shape, age band, and build are not — those carry the likeness.
  - Procedure detail and worked example: `references/cast-library-format.md`.
...
- State body proportions numerically in the `character_sheet` slot prompt, not as the phrase `adult proportions`. Left alone the model draws the sheet at 5-6 head heights — big head, short legs — which misreads the cast's age and, when they are real people, reads as a caricature of them. The `FIGURE PROPORTION LOCK` block (7.5 head heights, hip line at the figure's midpoint, knee at 28%, full body uncropped) belongs to the sheet slot only; pages inherit the correction through the sheet reference, so page prompt text stays untouched. Numbers, negative additions, and the measurement check: `references/adult-learning-comic-image-rules.md`.
...
- The explainer states mechanisms literally; the learner and challenger carry the humor. Every explainer bubble that carries a mechanism or claim must be true of the system when read word for word: the actual subject, the actual behavior, the condition it holds under. `LLM은 같은 모델이 생성한 텍스트를 더 후하게 평가하는 경향이 있어요` passes; `이름표가 보이면 후해지니까 떼고, 심판은 다른 집에서 데려와요` fails, because the reader has to translate `이름표` and `다른 집` back into the domain before the fact appears. Explainer analogies: at most one per page and three per series, each after the literal line it illustrates. Reference-material inserts, diagram labels, and page titles use the domain's real terms. Visual metaphors in the diagram (two chairs, a ruler) are fine; what the explainer says while pointing at them names the thing, not the picture. Rules: Explainer Direct Statement in `references/korean-copy-voice-rules.md`.
- The explainer states mechanisms literally; the learner and challenger carry the humor. Every explainer bubble that carries a mechanism or claim must be true of the system when read word for word: the actual subject, the actual behavior, the condition it holds under. `LLM은 같은 모델이 생성한 텍스트를 더 후하게 평가하는 경향이 있어요` passes; `이름표가 보이면 후해지니까 떼고, 심판은 다른 집에서 데려와요` fails, because the reader has to translate `이름표` and `다른 집` back into the domain before the fact appears. Explainer analogies: at most one per page and three per series, each after the literal line it illustrates. Reference-material inserts, diagram labels, and page titles use the domain's real terms. Visual metaphors in the diagram (two chairs, a ruler) are fine; what the explainer says while pointing at them names the thing, not the picture. Rules: Explainer Direct Statement in `references/korean-copy-voice-rules.md`.
...
- The same pass runs the explainer direct-statement check: mechanism and claim bubbles spoken by the explainer pass the substitution test (literally true of the system), explainer analogies stay within one per page and three per series, and material inserts, labels, and titles use domain terms. Learner, challenger, and mascot lines are not subject to the analogy budget.
- The same pass runs the explainer direct-statement check: mechanism and claim bubbles spoken by the explainer pass the substitution test (literally true of the system), explainer analogies stay within one per page and three per series, and material inserts, labels, and titles use domain terms. Learner, challenger, and mascot lines are not subject to the analogy budget.
...
11. `imagegen-checklist.md` records the Korean copy voice pass with zero remaining S1 patterns and the explainer direct-statement check (analogy count per page and per series), and for `adult-learning-comic` the character bible's Voice Lock names a speech level and ending set per character plus the explainer's direct-statement mode.
