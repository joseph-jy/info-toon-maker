...
`vocabulary`에 `비유 중심`, `explanation_devices`에 `비유`가 적힌 인물을 설명자로 캐스팅해도
설명자 직접 서술 규칙(`references/korean-copy-voice-rules.md`)은 그대로다. `voice`는 화계와 말버릇을
정하고, 직접 서술 규칙은 설명자 문장이 무엇을 주장하는지를 정한다. 비유를 좋아하는 인물은 페이지당
허용된 비유 1개를 쓰고, 메커니즘을 말하는 직접 서술 문장은 그대로 쓴다. 그 성향은 학습자·반론자로
캐스팅될 때 제한 없이 살아난다.
...
4. 승인된 캐스팅의 `identity_tokens`를 `character-bible.md`의 Identity Tokens 절에
   **그대로** 복사한다. 런 안에서 임의로 바꾸지 않는다.
   ...
   - **부분 프로필(필드가 일부만 채워진 경우)**: 위 필드 표가 `expertise`와 `speech_level`을
     필수로 표기하지만 `status: draft` 프로필은 그것들이 비어 있을 수 있다. 실재하는
     상태이므로 결함으로 취급하지 않는다. 처리 규칙은 **필드 단위**다.
     - 채워진 블록은 verbatim 복사한다. `identity_tokens`가 채워져 있으면 그것을 쓴다 —
       `voice`가 비어 있다는 이유로 얼굴·머리·의상까지 새로 설계하면 그 인물의 닮음을
       버리는 것이고 verbatim 규칙 위반이다.
     - 빈 블록만 이 런에서 설계한다. 없는 것을 만드는 것이지 있는 것을 고치는 게 아니다.
     - 어느 필드를 복사했고 어느 필드를 설계했는지 `character-bible.md`에 명시하고,
       프로필 파일은 편집하지 않았다는 사실을 함께 적는다. 다음 런이 같은 판단을
       다시 하지 않아도 되게 만드는 게 목적이다.
     - `catchphrases`가 비어 있으면 비운 채로 둔다. 없는 말버릇을 발명하면 그 인물의
       목소리가 아니라 이 하네스의 목소리가 된다.
     - 브리프나 지시가 "이 프로필은 비어 있으니 설계하라"고 뭉개서 말하더라도 파일을
       직접 열어 어느 블록이 비었는지 확인한다. 지시가 프로필 상태를 잘못 요약할 수 있다.
