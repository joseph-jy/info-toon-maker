...
- the Korean copy voice pass is recorded in `## Korean Copy Voice Pass` with zero remaining S1 patterns
- the character bible's Voice Lock names a speech level and ending set per character, and the pages hold them
- series-wide counts hold: English gloss on first use only, parallelism at most once, `X에서 Y로` at most once, colon-subtitle titles never
- no voice fix upgraded a claim: hedges required by `party-claim`, `analysis`, or `speculation` are still present
