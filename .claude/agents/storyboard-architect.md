...
- 설명자 말풍선은 직접 서술로 쓴다. 메커니즘·주장은 실제 대상을 주어로 실제 동작과 조건을 분야 용어로 말하고, 비유·치환어는 학습자·반론자·마스코트에게 준다. 설명자 비유는 페이지당 최대 1개, 직접 서술 뒤에만. 자료 인서트·다이어그램 라벨·페이지 제목은 분야 용어. Voice Lock의 설명자 항목과 `learning-design.md` Accuracy Gates에 기록한다. 규칙: `references/korean-copy-voice-rules.md`의 "설명자 직접 서술".
