...
7. Lock voice before writing page copy, then run one voice pass before freezing the whitelist. Follow `references/korean-copy-voice-rules.md` in the repository root. Run the spelling-robustness pass at the same time, per `references/korean-baked-text-spelling-rules.md` — fragile strings get replaced while the copy is still soft, because after a page renders a string fix costs that render.
   ...
   - The explainer states mechanisms literally. Every explainer bubble that carries a mechanism or claim must be true of the system when read word for word: the actual subject (`LLM`, `심판 모델`, `프롬프트`), the actual behavior, the condition. Write `LLM은 같은 모델이 생성한 텍스트를 더 후하게 평가하는 경향이 있어요`, not `이름표가 보이면 후해지니까 떼고, 심판은 다른 집에서 데려와요`. Explainer analogies are budgeted at one per page and three per series, each after its literal line; a claim-ledger `allowed wording` written as figurative shorthand does not lift the budget. Reference-material inserts, diagram labels, and page titles use the domain's real terms. The learner and challenger keep their figurative, casual lines — the series stays light through them, not through the explanation. Rule and examples: Explainer Direct Statement in `references/korean-copy-voice-rules.md`.
