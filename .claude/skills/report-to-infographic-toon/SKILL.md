...
7. Lock voice before writing page copy, then run one voice pass before freezing the whitelist. Follow `references/korean-copy-voice-rules.md` in the repository root.
...
6. For a learning comic, render `character_sheet` and `page_01` first. Inspect both before rendering the remaining pages and the landscape `thumbnail` with the same character sheet reference. The thumbnail renders automatically in `--mode series` (default size `1536x1024`; override with `--thumbnail-size` or `OPENAI_THUMBNAIL_SIZE`).
